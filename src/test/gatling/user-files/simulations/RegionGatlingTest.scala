...
class RegionGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Region entity")
        ...
        .repeat(2) {
            ...
            .get("/services/simplejobhistory/api/regions")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new region")
            .post("/services/simplejobhistory/api/regions")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "regionName":"SAMPLE_TEXT"
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/services/simplejobhistory${new_region_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created region")
            .delete("/services/simplejobhistory${new_region_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
